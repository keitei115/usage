...
public interface PokeRepository extends JpaRepository<Pokemon, String> {
    ...
    List<ItemTotal> getItemTotal(String name, LocalDate before, LocalDate after);
    ...
    List<MoveTotal> getMoveTotal(String name, LocalDate before, LocalDate after);

    @Query(value = "SELECT nature, COUNT(nature) / (SELECT CAST(COUNT(nature) AS REAL) FROM pokemon WHERE name = ?1 AND date BETWEEN ?2 AND ?3) * 100.0 AS naturetotal FROM pokemon WHERE name = ?1 AND date BETWEEN ?2 AND ?3 GROUP BY nature ORDER BY naturetotal DESC, nature LIMIT 25", nativeQuery = true)
    List<NatureTotal> getNatureTotal(String name, LocalDate before, LocalDate after);

    @Query(value = "SELECT ability, COUNT(ability) / (SELECT CAST(COUNT(ability) AS REAL) FROM pokemon WHERE name = ?1 AND date BETWEEN ?2 AND ?3) * 100.0 AS abilitytotal FROM pokemon WHERE name = ?1 AND date BETWEEN ?2 AND ?3 GROUP BY ability ORDER BY abilitytotal DESC, ability LIMIT 5", nativeQuery = true)
    List<AbilityTotal> getAbilityTotal(String name, LocalDate before, LocalDate after);

    @Query(value = "SELECT name,CAST(COUNT(name) AS REAL) / (SELECT COUNT(partyid) FROM POKEMON WHERE name = ?1 AND date BETWEEN ?2 AND ?3) * 100.0 AS pokemontotal FROM POKEMON WHERE date BETWEEN ?2 AND ?3 AND partyid IN (SELECT partyid FROM pokemon WHERE NAME = ?1) AND name != ?1 GROUP BY name ORDER BY pokemontotal DESC, name LIMIT 20", nativeQuery = true)
    List<PokemonTotal> getPokemonTotalBySameParty(String name, LocalDate before, LocalDate after);
    ...
    float getPokemonUsage(String name, LocalDate before, LocalDate after);
